...
namespace ApiConsume.Areas.Airports.Controllers
{
    [CheckAccess]
    [Area("Airports")]
    ...
    public class AirportsController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage responseMessage = await client.GetAsync("Airports/Get");
                if (responseMessage.IsSuccessStatusCode)
                {
                    string data = responseMessage.Content.ReadAsStringAsync().Result;
                    AirportListResponse jsonObject = JsonConvert.DeserializeObject<AirportListResponse>(data);
        ...
        public DataTable AirportList()
        {
            ...
            using (var client = new HttpClient()) // Corrected variable name from _client to client
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage response = client.GetAsync($"{client.BaseAddress}Airports/Get").Result; // Corrected variable name from _client to client
        ...
        public IActionResult Export_Airport_List_To_Excel()
        {
            DataTable dataTable = AirportList();
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    ...
                    {
                        IXLWorksheet ws = wb.Worksheets.Add("Airport Statements");
                        ...
                        string fileName = "Airport_Lists.xlsx";
        ...
        public IActionResult Export_Airport_List_To_pdf()
        {
            DataTable dataTable = AirportList();
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    ...
                    using (Document document = new Document(customPageSize))
                    {
                        ...
                        // Title
                        Paragraph title = new Paragraph("Airport List", new Font(boldBaseFont, 35));
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage responseMessage = await client.GetAsync("Airports/Get/" + id);
                if (responseMessage.IsSuccessStatusCode)
                {
                    string data = responseMessage.Content.ReadAsStringAsync().Result;
                    AirportsResponse jsonObject = JsonConvert.DeserializeObject<AirportsResponse>(data);
                    var dataOfObject = jsonObject.data;
                    return View("InsertAirports", dataOfObject);
            ...
            return View("InsertAirports");
        }

        public async Task<IActionResult> Insert(AirportsModel AirportsResponse)
        {
            using (var client = new HttpClient())
            {
                Console.WriteLine(AirportsResponse);
                client.BaseAddress = new Uri(baseurl);
                string data = JsonConvert.SerializeObject(AirportsResponse);
                StringContent content = new StringContent(data, Encoding.UTF8, "application/json");
                if (AirportsResponse?.airport_id == null)
                {
                    HttpResponseMessage responseMessage = await client.PostAsync("Airports/Insert", content);
                    if (responseMessage.IsSuccessStatusCode)

                    {
                        TempData["Message"] = "Airport added successfully";
                ...
                {
                    HttpResponseMessage responseMessage = await client.PutAsync("Airports/Update", content);
                    if (responseMessage.IsSuccessStatusCode)
                    {
                        TempData["Message"] = "Airport updated successfully";
                        return RedirectToAction("Index");
                    }
                    else
                    {
        ...
        public async Task<IActionResult> DeleteAirport(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage responseMessage = await client.DeleteAsync("Airports/Delete?airport_id=" + id);
                if (responseMessage.IsSuccessStatusCode)
                {
                    TempData["Message1"] = "Airport deleted successfully";
                    return RedirectToAction("Index");
