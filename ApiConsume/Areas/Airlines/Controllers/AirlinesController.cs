...
namespace ApiConsume.Areas.Airlines.Controllers
{
    [CheckAccess]
    [Area("Airlines")]
    ...
    public class AirlinesController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage responseMessage = await client.GetAsync("Airlines/Get");
                if (responseMessage.IsSuccessStatusCode)
                {
                    string data = responseMessage.Content.ReadAsStringAsync().Result;
                    AirlinesListResponse jsonObject = JsonConvert.DeserializeObject<AirlinesListResponse>(data);
        ...
        public DataTable AirlineList()
        {
            ...
            using (var client = new HttpClient()) // Corrected variable name from _client to client
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage response = client.GetAsync($"{client.BaseAddress}Airlines/Get").Result; // Corrected variable name from _client to client
        ...
        public IActionResult Export_Airline_List_To_Excel()
        {
            DataTable dataTable = AirlineList();
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    ...
                    {
                        IXLWorksheet ws = wb.Worksheets.Add("Airline report  ");
                        ...
                        string fileName = "Airline_Lists.xlsx";
        ...
        public IActionResult Export_Airline_List_To_pdf()
        {
            DataTable dataTable = AirlineList();
            ...
            if (dataTable != null && dataTable.Rows.Count > 0)
            {
                using (MemoryStream memoryStream = new MemoryStream())
                {
                    ...
                    using (Document document = new Document(customPageSize))
                    {
                        ...
                        // Title
                        Paragraph title = new Paragraph("Airline List", new Font(boldBaseFont, 35));
        ...
        public async Task<IActionResult> Edit(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage responseMessage = await client.GetAsync("Airlines/Get/" + id);
                if (responseMessage.IsSuccessStatusCode)
                {
                    string data = responseMessage.Content.ReadAsStringAsync().Result;
                    AirlinesResponse jsonObject = JsonConvert.DeserializeObject<AirlinesResponse>(data);
                    var dataOfObject = jsonObject.data;
                    return View("InsertAirlines", dataOfObject);
            ...
            return View("InsertAirlines");
        }

        public async Task<IActionResult> Insert(AirlinesModel AirlinesResponse)
        {
            using (var client = new HttpClient())
            {
                Console.WriteLine(AirlinesResponse);
                client.BaseAddress = new Uri(baseurl);
                string data = JsonConvert.SerializeObject(AirlinesResponse);
                StringContent content = new StringContent(data, Encoding.UTF8, "application/json");
                if (AirlinesResponse?.airline_id == 0)
                {
                    HttpResponseMessage responseMessage = await client.PostAsync("Airlines/Insert", content);
                    if (responseMessage.IsSuccessStatusCode)
                    {
                        TempData["Message"] = "airline  added successfully";
                ...
                {
                    HttpResponseMessage responseMessage = await client.PutAsync("Airlines/Update", content);
                    if (responseMessage.IsSuccessStatusCode)
                    {
        ...
        public async Task<IActionResult> DeleteAirline(int id)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                client.DefaultRequestHeaders.Accept.Clear();
                client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                HttpResponseMessage responseMessage = await client.DeleteAsync("Airlines/Delete?airline_id=" + id);
                if (responseMessage.IsSuccessStatusCode)
                {
                    TempData["Message1"] = "Airline  deleted successfully";
                    return RedirectToAction("Index");
        ...
        [HttpGet]
        public async Task <IActionResult> Delete(string[] airline_idList)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                if (airline_idList != null && airline_idList.Length > 0)
                {
                    // Convert the array of IDs to a comma-separated string
                    string idList = string.Join(",", airline_idList);


                    // Call the API to delete the multiple employees
                    client.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                    HttpResponseMessage responseMessage = await client.DeleteAsync("api/Airlines/MultipleDelete?airline_id" + idList);
